...
class JLChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/gt/jl_challenge/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/jl_challenge/'),  # Trackers location
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['airplane','ship']
        # self.valid_classes = ['pedestrian']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        self.distractor_class_names = self.valid_classes
        self.distractor_class_names.remove(self.class_list[0])
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only pedestrian class is valid.')
        self.class_name_to_class_id = {'airplane': 1, 'ship': 2}
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        distractor_class_names = self.distractor_class_names
